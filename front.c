...
static int next_memory_address = 0x1000; /* Starting address set to 0x1000 */
/* Instruction table */
...
/* Function to check if a string is a macro */
...
int is_macro(const char *str) {
    ...
    while (m != NULL) {  /* Iterate through the macro list */
        if (strcmp(m->name, str) == 0) {
            return 1;  /* Return 1 if the macro is found */
        }
        m = m->next;
    }
    return 0;  /* Return 0 if the macro is not found */
}

/* Function to find a variable's address */
/* Returns the address if found, -1 otherwise */
int find_variable(const char *name) {
    int i;
    for ( i = 0; i < variable_count; i++) {  /* Iterate over the variable list */
        if (strcmp(variables[i].name, name) == 0) {
            return variables[i].address;  /* Return the variable's address if found */
        }
    }
...
int add_variable(const char *name) {
    int address;
    if (variable_count >= MAX_VARIABLES) {  /* Check if the variable table is full */
        fprintf(stderr, "Error: Too many variables.\n");
        exit(1);  /* Exit with an error if the table is full */
    }
    address = next_memory_address++;  /* Assign the next memory address to the variable */
    strcpy(variables[variable_count].name, name);  /* Copy the variable name */
    variables[variable_count].address = address;  /* Assign the address to the variable */
    variable_count++;  /* Increment the variable count */
    return address;  /* Return the variable's address */
}
